...
#     # print(original_sentences);exit()
#     filtered_sentences = filter_words(original_sentences)

#     return merge_syntactic_units(original_sentences, filtered_sentences)

def clean_text_by_sentences(text, embedding,language="english", additional_stopwords=None, algorithm='string'):
    """ Tokenizes a given text into sentences, applying filters and lemmatizing them.
    Returns a SyntacticUnit list. """
    if algorithm =='string':
        init_textcleanner(language, additional_stopwords)
    ...
    if algorithm == 'list':
        ...
        # print(original_sentences);exit()
        if embedding == 'tfidf':
            filtered_sentences = filter_words(text)
            return merge_syntactic_units(text, filtered_sentences)
        if embedding == 'simcse':
            return merge_syntactic_units(text, text)
