...
def build_my_test_cid_query_dicts():
    # cid & query
    results  = []
    cids = []
    datapath = '/workspace/deepsummarize/query_sum/dataset/input/json'
    for item in os.listdir(datapath):
        final = {}
        result = item.split('_') 
        final['cid']= result[0]+'_'
        final['query']=result[1][:-4]
        results.append(final)
        cids.append(result[0])
    # print(results);exit()
    return results,cids
...
def rank_e2e():
    ...
    rank_dp = '/workspace/deepsummarize/query_sum/baseline/corase_to_fine/querysum/rank/stackoverflow'
    ...
    if not exists(rank_dp):
        os.mkdir(rank_dp)
    test_cid_query_dicts,_ = build_my_test_cid_query_dicts()
    # print(test_cid_query_dicts);exit()
    ### test_cid_query_dicts 是query的二元组,
...
def ir_rank2records():
    ...
    ir_config.IR_RECORDS_DIR_NAME_TF = 'ir_records-ir-tf-stackoverflow-0.75_ir_conf'
    # print(ir_config.IR_RECORDS_DIR_NAME_TF);exit()
    ...
    _,cids = build_my_test_cid_query_dicts()
