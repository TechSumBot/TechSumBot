...
class DatasetParser:
    ...
    def cid2sents(self, cid, rm_dialog=True, rm_stop=True, stem=True, max_ns_doc=None):
        ...
        original_sents, processed_sents = [], []
        original_doc_sents = []
        documents_dir = '/workspace/deepsummarize/query_sum/dataset/input/json'
        for file in os.listdir(documents_dir): 
            ...
            if cid[:-1] ==mark:
                with open(os.path.join(documents_dir,file),'rb') as f:
                    ...
                    for doc in data:
                        sents = doc[3:]
                        original_sents.append(sents)
                        tmp = []
                        for original in sents:
                            processed_para_sents = self._proc_sent(original, rm_dialog=False, rm_stop=True, stem=True)
                            tmp.append(processed_para_sents)
                        processed_sents.append(tmp)                                                                    
        ...
        #     # original doc 是原句的list, processed 是预处理的结果
        ...
        #     # print(processed_doc_sents)
        #     # print(max_ns_doc);exit()                                                                               
